...
class AdminUserView extends Controller {
    ...
    public function index() {
        ...
        /* Get the websites total */
        $user_websites_total = Database::$database->query("SELECT COUNT(*) AS `total` FROM `websites` WHERE `user_id` = {$user_id}")->fetch_object()->total ?? 0;
        ...
        /* Get the user created teams */
        $user_teams_total = Database::$database->query("SELECT COUNT(*) AS `total` FROM `teams` WHERE `user_id` = {$user_id}")->fetch_object()->total ?? 0;

        /* Get the user created teams */
        $user_teams_associations_total = Database::$database->query("SELECT COUNT(*) AS `total` FROM `teams_associations` WHERE `user_id` = {$user_id}")->fetch_object()->total ?? 0;
        ...
        $data = [
            'user'                  => $user,
            'user_websites_total'  => $user_websites_total,
            'user_payments_total'  => $user_payments_total,
            'user_teams_total'  => $user_teams_total,
            'user_teams_associations_total'  => $user_teams_associations_total,
