...
<div class="mb-5 row justify-content-between">
    <div class="col-12 col-sm-6 col-xl-3 mb-4">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body">
                <small class="text-muted"><i class="fa fa-fw fa-sm fa-server mr-1"></i> <?= $this->language->admin_index->display->websites_month ?></small>

                <div class="mt-3"><span class="h4"><?= nr($data->websites->websites_month) ?></span></div>
            </div>
        </div>
    </div>
    ...
    <div class="col-12 col-sm-6 col-xl-3 mb-4">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body">
                <small class="text-muted"><i class="fa fa-fw fa-sm fa-server mr-1"></i> <?= $this->language->admin_index->display->websites ?></small>

                <div class="mt-3"><span class="h4"><?= nr($data->websites->websites) ?></span></div>
            </div>
...
<?php $result = \Altum\Database\Database::$database->query("SELECT `websites`.*, `users`.`name` AS `user_name` FROM `websites` LEFT JOIN `users` ON `websites`.`user_id` = `users`.`user_id` ORDER BY `website_id` DESC LIMIT 5"); ?>
<?php if($result->num_rows): ?>
    <div class="mb-5">
        <h1 class="h3 mb-4"><?= $this->language->admin_index->websites->header ?></h1>

        <div class="table-responsive table-custom-container">
            <table class="table table-custom">
                <thead>
                <tr>
                    <th><?= $this->language->admin_index->websites->website ?></th>
                    <th><?= $this->language->admin_index->websites->user ?></th>
                    <th></th>
                    <th></th>
                </tr>
                </thead>
                ...
                <?php while($row = $result->fetch_object()): ?>
                    <tr>
                        ...
                        <td><?= include_view(THEME_PATH . 'views/admin/partials/admin_website_dropdown_button.php', ['id' => $row->website_id]) ?></td>
                    </tr>
                <?php endwhile ?>
                </tbody>
            </table>
